...
- **Signing key lifetime.** Any deployment beyond a single local dev box must set `MACP_AUTH_SIGNING_KEY_JSON`. Without it the service generates an ephemeral RSA keypair on every start — the runtime's JWKS cache (default 60–300 s) will reject tokens minted before a restart.
- **Key rotation.** Generate a new JWK with a fresh `kid`, deploy, and wait one `MACP_AUTH_JWKS_TTL_SECS` window. In-flight tokens signed with the prior `kid` remain valid until they expire; new mints use the new key. There is no revocation list — keep TTLs short if rapid revocation matters.
