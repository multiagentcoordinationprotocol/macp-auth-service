...
| Caller sends a broken JSON body (shell quoting ate the braces) | `400 invalid JSON body` | Fix the body; this error is distinct from `sender is required`, so it always means the JSON itself is unparseable. |
| Caller passes `scopes` as a string or array (e.g. `"scopes": "admin"`) | `400 scopes must be an object` | Pass a JSON object. The runtime deserializes `macp_scopes` into a struct, so the mint boundary rejects non-objects rather than minting a token the runtime would refuse. |
